...
class Dalc:
    ...
    def learn(self, X_source, X_target, y_source, y_target, kernel=None, return_kernel_matrix=False):
        ...
        data_matrix = np.vstack((X_source, X_target))
        label_vector = np.hstack((y_source, np.zeros(len(y_target))))
    ...
    def perform_one_optimization(self, initial_vector, i):
        ...
        if self.verbose:
            ...
            for (key, val) in optimizer_output[2].items():
                if key != 'grad': print(str(key) + ': ' + str(val))                    
